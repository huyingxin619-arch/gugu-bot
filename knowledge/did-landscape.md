...
## CAID 版本归一化（2026-09-02 新增）

与徐小丹、谢品端讨论确认：

### 中台归一化方案
- 中台已有 CAID 版本归一化方案：利用同一日志携带多版本 CAID 的特性，构建版本关联，归并到统一 `mz_caid`，每日增量
- DMP 已在使用 `mz_caid` 表，**ADM 常规计算未使用此表**
- 应用价值：跨版本 CAID 归并可解决版本迭代期间的 ID 打通断裂问题

### CAID 稳定性问题
- 谢品端反馈：**同一 IDFA 对应多个同版本 CAID（一对多）**，一个月内变化频繁
- 可能原因：① 媒体使用缓存 IDFA（非实时获取）② IVT 影响导致异常 IDFA 产生多 CAID
- 待办：CBP 团队验证媒体是否使用缓存 IDFA（小胡跟进）

### 对 DID Landscape 的影响
- CAID 一对多问题意味着 CAID 作为设备标识的稳定性低于预期
- `mz_caid` 归一化方案是中台层面的解决路径，但 ADM 侧尚未接入
- 需关注：若 ADM 接入 `mz_caid`，对 UV 计算和跨媒体打通的影响
