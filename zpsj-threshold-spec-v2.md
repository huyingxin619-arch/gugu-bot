...
2. **设备截图验证**：将推导出的阈值应用于真实设备截图数据，验证实际通过率与假阳性率
...
![正样本H分布直方图](scripts/zpsj-charts/01_pos_h_dist.png)
...
![正样本Cos分布直方图](scripts/zpsj-charts/02_pos_cos_dist.png)
...
![正负样本H分布对比](scripts/zpsj-charts/05_h_comparison.png)
...
![正负样本Cos分布对比](scripts/zpsj-charts/06_cos_comparison.png)
...
![正负样本H-Cos二维散点图](scripts/zpsj-charts/10_scatter_2d.png)

将两个维度同时展开，观察正负样本在 H-Cos 二维空间中的分布：

![参数空间热力图](scripts/zpsj-charts/07_param_space.png)

**关键观察**：

- **正样本**聚集在左上角（低 H、高 Cos），形成一个紧凑的簇
...
1. **取分离带的正样本侧边界值**：保证 100% 正样本覆盖
2. **两个维度同时约束**：利用互补性弥补单维度的盲区
...
![H阈值与通过率曲线](scripts/zpsj-charts/08_h_passrate.png)

从曲线可以看到通过率随 H 阈值的变化趋势：H=10 处正样本通过率达到 100%，而负样本（尤其是 N4）的通过率在 H=10 之前已经降至很低。曲线的拐点正出现在 H=10–12 附近，与分离带位置一致。
...
N2 的 4.2% 假阳性全部来自同一素材（v05 黄昏高架），该素材近乎静态（帧间差异极小），导致远距帧的 H 和 Cos 都接近正样本。这是一个边缘案例，可通过素材侧标注（标记近乎静态的素材为高风险）来处理。
...
![滑动窗口累计挽回率](scripts/zpsj-charts/09_sliding_window.png)

从数据看：
- ±50ms 窗口可挽回 15 个点，通过率从 85.8% 提升到 95.1%
...
依据：
- ±100ms 已覆盖所有可挽回的同 CF 点，通过率达 100%
...
| 滑动窗口 | ±100ms（数据覆盖）/ ±400ms（生产保守） | ±100ms 时同 CF 累计通过率达 100%，之后无新增；±400ms 为生产环境留余量 |
...
1. **暗色低对比素材**（A1）：HSV 直方图信噪比低，通过率 66.7%，需针对性处理
2. **近乎静态素材**（如 v05 黄昏高架）：远距帧 H 和 Cos 均接近正样本，N2 假阳性率 4.2%，需素材侧标注风险
3. **阈值假设正负样本分离带在当前编码链路下成立**，若编码链路变化需重新评估
