...
- **踩坑**: 2026-08-06，cron job model ID 需跟随网关格式变更同步。原 memory-distill job 用旧模型ID `mlamp/kimi-k2.6`（已不可用），连续4天 503 失败（"无可用渠道"）。修复：model 改为 `tencent/glm-5.2`。教训：mlamp 网关模型ID格式变更后，所有 cron job 的 model 需同步检查更新
