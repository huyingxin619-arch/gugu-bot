...
### Bug-03：N+指标默认值不一致
- **现象**：固定模板不传 NPlus/reachN 报 40001（参数缺失），但自定义模板有默认值（不传也能创建成功）
- **结论**：改文档而非改系统——固定模板和自定义模板的校验逻辑不同是设计行为，文档需补充说明
- **待办**：李晴晴确认文档修改

### Bug-04：ADM定制模板不传 TimeSet 也能创建成功
- **现象**：界面强制要求 TimeSet，但 API 创建 ADM 定制模板时不传 TimeSet 也能成功
- **结论**：API 校验缺失，需修复
- **待办**：黄春波明天修完（8/20）
