...
> **状态：** v0.3 — 已正式立项，技术开发中（预计2周），本文档作为风险排查清单
> **上次更新：** 2026-07-17 11:50  
...
> **定位调整（7/17）：** 技术已启动开发，本文档价值在于暴露易遗漏细节，非技术实施的前置输入
...
> ⚠️ 以下为技术开发中容易遗漏的细节，建议逐条确认。

### 14.1 界面回显与同步对齐

**问题来源：** 小胡（产品方）明确提出，非业务侧需求。

| 风险点 | 说明 | 建议 |
|--------|------|------|
| 模板参数↔界面对应 | `template_id` 和定制模板的 row/column 组合，必须能反向映射回界面的模板选择和维度拖拽状态 | 要求技术做一次 API 参数 → 界面渲染的双向验证 |
| 维度过滤条件回显 | `filters.regions` 等过滤条件，界面上要能显示"已筛选：北京、上海" | 确认 filters 结构包含 label + value，不仅是 ID |
| 时间粒度回显 | `granularity=day` 在界面上对应"按日"选项 | 确认 API 粒度值与界面下拉项一一对应 |
| 平台类型回显 | `platform=mb` 在界面上对应"移动端" tab | 确认平台字段能映射到界面 tab 状态 |
| Table 拆分回显 | 多 table 结果在界面上如何展示？当前界面是分页还是分 tab？ | 确认 API 返回的 multi-table 结构与界面展示方式一致 |
| **校验逻辑同步** | 网页前端有维度/指标校验（如MB活动不能选device维度），API必须走同一套校验逻辑，不能绕过 | 确认API复用前端校验规则，或统一走后端校验服务 |
| **区分AI和人提的报告** | 需要能统计AI使用比例 | API请求增加 `source` 字段（如 `ai`/`manual`），任务记录中标注来源 |
| **界面更新API同步更新** | 界面加了新维度/指标/模板，API必须同步支持 | 建立联动机制：界面需求变更时强制check API是否需要同步 |

### 14.2 权限适配

**问题：** API 的权限模型必须与现有界面权限完全一致，否则会出现"界面能看但 API 不能查"或反向的安全漏洞。

| 风险点 | 说明 | 建议 |
|--------|------|------|
| 活动级权限 | 界面上用户能看到哪些活动，API 也只能查这些活动 | 复用现有 show API 的活动授权校验逻辑，不要新建权限体系 |
| 数据维度权限 | 某些维度是否对部分用户不可见？如 Demography | v1 不做 Demography，但仍需确认基础维度是否有权限分级 |
| 跨活动查询权限 | 用户一次提交多个 activity_ids，每个活动的权限需独立校验 | 逐个校验，任一无权限则整个请求拒绝（不要部分成功） |
| API Key 归属 | API Key 绑定的用户身份，其权限边界需与登录用户一致 | 确认 OAuth2 token 解析出的用户上下文与界面登录用户走同一套权限引擎 |
| 数据脱敏一致性 | 界面上活动名称/客户名称是否脱敏？API 返回是否一致？ | 确认脱敏规则在 API 和界面是同一套逻辑 |

### 14.3 数据逻辑对齐

| 风险点 | 说明 | 建议 |
|--------|------|------|
| 指标计算口径 | API 返回的指标值必须与界面展示的值完全一致 | 用同一套计算逻辑，不要 API 单独算一遍 |
| 衍生指标 | CTR = Click/Imp，reach_pct = reach/universe×100% | 确认衍生指标的计算公式在 API 和界面是同一套 |
| sample_size < 300 标红 | 界面上标红显示，API 怎么表达？ | 建议返回 `is_low_sample: true` 标记 |
| 累计 vs 汇总 | `granularity=acc` 和 `granularity=total` 的区别要明确 | 确认技术理解与界面行为一致 |
| 时区 | 时间范围按哪个时区解析？ | 明确 Asia/Shanghai，在文档和接口中都标注 |
| 空值处理 | 某些维度/指标在某些活动中没有值（如 PM 活动没有 App 维度） | 返回 null + 说明，不要报错 |

### 14.4 异步任务工程风险

| 风险点 | 说明 | 建议 |
|--------|------|------|
| 任务队列选型 | 用 Redis / MQ / 数据库轮询？影响可靠性和成本 | 参考现有 M+ 的异步模式，优先复用 |
| 并发控制 | 5 个并发限制是单用户还是全局？ | 明确：建议单用户 5 个，全局另设上限 |
| 超时处理 | 15 分钟大查询如果超时，错误信息怎么给？ | DRILL_502 已定义，确认技术能区分"引擎故障"和"数据量过大" |
| 结果存储 | 结果存哪里？7 天过期怎么清理？ | 建议用对象存储 + TTL，不要存数据库 |
| 幂等实现 | 60 秒内相同参数返回同一任务，怎么判断"相同"？ | 需定义参数指纹算法（排序+hash），注意 JSON 字段顺序问题 |
| 回调可靠性 | Webhook 回调失败怎么办？ | v1 可选回调，失败不影响主流程；但需记录回调失败日志 |
| **大任务自动拆分** | 点位过多/结果行数过多时，系统应自动拆分为子任务分批计算，汇总结果输出 | 需定义拆分阈值（按点位数？预估行数？）、拆分策略、子任务并行度、汇总合并逻辑 |
| **拆分结果合并** | 分批算的结果如何合并？合计行怎么算？ | 需明确合并规则：同维度合并求和，衍生指标（CTR等）重新计算而非简单加总 |

### 14.5 边界场景

| 场景 | 预期行为 | 是否已覆盖？ |
|------|----------|-------------|
| 活动无数据（曝光量为0） | 返回空结果 + 摘要"该活动在所选时间范围内无数据" | ❓ 需确认 |
| 维度组合导致结果为空 | 如筛选了某地域但该地域无数据 | 返回空表 + 说明 |
| 时间范围跨活动周期 | 活动已结束但查询历史数据 | 应正常返回，确认数据保留期限 |
| 多活动混合平台 | 一次提交 PC + MB 活动 | 应报错还是拆分查询？建议报错 DRILL_003 |
| 结果超 100 万行 | truncated + 提示拆分 | 确认截断逻辑和提示信息 |
