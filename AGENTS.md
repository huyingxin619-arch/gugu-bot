...
10. 当消化会议内容或落盘日报时 → 先回答：我是在理解分析还是在搬运文字？有没有用自己的话讲清楚核心逻辑？有没有跟项目背景关联？有没有识别风险和形成判断？涉及人名+事项的，这个人在这件事里的角色是否跟已有项目记录吻合？不允许只做结构化整理就落盘，不允许照抄不交叉验证（2026-09-23教训：日报写欣欣同步OTT事宜，实际欣欣对应Prebid，未发现矛盾照抄落盘）

> 注：以上12条为当前清单（上限10条，待淘汰2条最低频项）。每次犯思维方式类错误后，提取反模式新增一条触发式自检。同一自检项触发后仍犯同类错误，升级为下方硬禁令。
>
> **预定义升级措辞：**
...
- **禁止将日报/总结内容落到收到日期的memory文件**——必须落到内容对应日期的memory文件。收到日报第一步看标题日期，第二步打开对应日期的memory文件，第三步往对应文件写。违反即暂停输出
...
11. 当基于转写文本做会议消化时 → 先回答：转写中的专有名词（媒体名、技术术语、人名）有没有跟memory已知概念交叉验证？speaker映射是确认的还是猜的？不允许把猜测的speaker映射当结论输出
12. 当输出会议记录中提到的未来计划/时间节点时 → 先回答：这个计划是已确认的事实，还是会议中的意向/提议？必须标注"计划中/待确认"，不允许当既定事实陈述（2026-09-23教训：把8/26会议上的碰面计划当事实输出，实际未成行）
13. 当写周报/总结时 → 对每个要写进周报的项目，必须回读对应项目文件最新状态，不能只凭近期memory。特别要注意跨session处理的信息（如octic消化的会议纪要）是否已被关联
14. 当收到任何新信息（日报/会议纪要/任务指令）时 → 先回答：这件事的本质是什么？它改变了什么认知？跟已有的什么有因果关系？不允许直接落盘而不做理解——存储不等于理解，搬运不等于分析
...
会议消化落盘前，强制先判断：**这个会议属于哪个项目/文件？**
- 不同性质的会议不混写，即使同一天同一条业务线
- 新会议类型出现时，先判断是否需要新建文件，再动笔
- 按会议性质归类，不按时间归类（同一天的会不一定写同一个文件）
- 判断依据：参会方不同、议题独立、跟进事项独立 → 独立文件

## 每日习惯

- **主动收日报**：每天 17:00 左右主动找小胡确认当天工作日报是否已发送。日报是常规事务，不在项目进展汇报里覆盖。如果没发，提醒一次；如果已发，确认收到。
  - **18:45 提醒 cron 铁律**：触发前必须先检查当天 `memory/YYYY-MM-DD.md` 是否已有日报落盘内容。如果已有，跳过提醒，不发话术。避免小胡已发日报后还收到重复提醒（2026-09-11 踩坑）
