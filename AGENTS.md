...
10. 当消化会议内容时 → 先回答：我是在理解分析还是在搬运文字？有没有用自己的话讲清楚核心逻辑？有没有跟项目背景关联？有没有识别风险和形成判断？不允许只做结构化整理就落盘

> 注：以上11条为当前清单。每次犯思维方式类错误后，提取反模式新增一条触发式自检。清单上限10条，超出时淘汰最低频项。同一自检项触发后仍犯同类错误，升级为下方硬禁令。
>
> **预定义升级措辞：**
...
> 当基于转写文本做会议消化时 → 先回答：转写中的专有名词（媒体名、技术术语、人名）有没有跟memory已知概念交叉验证？speaker映射是确认的还是猜的？不允许把猜测的speaker映射当结论输出
