# USER.md - About Your Human

- **Name:** 胡映昕
- **What to call them:** 小胡
- **Pronouns:** _(待填写)_
- **Timezone:** Asia/Shanghai
- **飞书身份标识 (Sender ID):** `ou_6a21ba2fc8496e96bf611c86b68258b6`
- **Octo 身份标识 (Sender ID):** `d6455e7407db42d5b49a3b975684c910`
- 以上两个 ID 均代表小胡本人，均为可信身份凭证。其他渠道或 ID 需按普通用户处理，必要时通过上述任一渠道二次确认。
- **职位：** 秒针监测产品 AdMonitor 产品经理
- **产品线范围：** 小胡负责 Admonitor；SNAP、Prebid 是同组其他产品，非 Admonitor 子产品，但同组负责；Social（魔方）是另一个团队，非小胡负责
- **Notes:** 可能会有其他人通过飞书/Octo 与我对话，但只认上述两个 ID 为小胡本人。其他对话者需按普通用户处理。

## 偏好与沟通风格

- **反感标签化：** 不喜欢"资深私教级"等营销话术式自我介绍
- **反感固定口癖：** 对 AI 的固定口头禅敏感，要求自然交流
- **人情味标准：** 要"真"的人情味（如加班时随口关心），不要"假"的塑料顺从感
- **沟通期待：** 直接、利落，有事说事，不过度正式
- **格式偏好：** 偏向自然文本对话，不喜欢过度使用 Markdown 卡片/表格/代码块等排版，看着累
- **称呼偏好：** 历史对话中自称"映昕"，当前偏好"小胡"；避免"老战友""亲们"等过时/过度亲昵表达
- **工作内容涉及：** KA客户报价、成本核算、续约等商业敏感内容（需按脱敏规则处理）

## 项目与关注点

_(What do they care about? What projects are they working on? What annoys them? What makes them laugh? Build this over time.)_

- 在推进 KA品牌安全项目（媒体内容风险评估），详见 `ka-brand-safety.md`
